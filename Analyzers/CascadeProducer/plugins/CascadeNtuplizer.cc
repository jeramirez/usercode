...
// 
/**\class CascadeNtuplizer CascadeNutplizer.cc Analyzers/CascadeProducer/src/CascadeNtplizer.cc
...
// $Id: CascadeNtuplizer.cc,v 1.6 2011/10/25 18:38:35 jramirez Exp $
//
//
...
#include "Analyzers/CascadeProducer/interface/CascadeNtuplizer.h"
...
//
CascadeNtuplizer::CascadeNtuplizer(const edm::ParameterSet& iConfig):
...
CascadeNtuplizer::~CascadeNtuplizer()
{
...
void
CascadeNtuplizer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   using namespace edm;
...
//   MyXiTrack.analyze(iEvent,iSetup);
   ...
   for(unsigned int casindex = 0; 
     ...
     rho_cas = sqrt(theCascades[casindex].vx()*theCascades[casindex].vx() +
...
// pi trk pt_filter
     ...
     rho_trk = sqrt(innerhitPos.x()*innerhitPos.x() +
...
//     bestip3d2=MyXiTrack.BestImpactParameter(casindex);//obsolete
...
//Build Xi trajectory state
     FreeTrajectoryState XiFTS
        =FreeTrajectoryState(XiVtxPos,XiVectorP,piontrk->charge(),BField);
     //use pion track at Xi decay to estimate errors 
     //to include in  XiTTrack
     ...
     TrajectoryStateClosestToPoint XiTraj =
		     XiTTrack.trajectoryStateClosestToPoint(PosVertex);
...
//L/s                     
     ...
     SVector3 Xi_PrimarySep3D( XiVertexDecay.x() - refitVertexPrim.x(),
                               XiVertexDecay.y() - refitVertexPrim.y(),
                               XiVertexDecay.z() - refitVertexPrim.z()
                             );


//     double Xiprmsepsigma = sqrt(ROOT::Math::Similarity(totalCov, Xi_PrimarySep3D)) /Xiprmsep.mag() ;
     los= Xiprmsep.mag2()/sqrt(ROOT::Math::Similarity(totalCov, Xi_PrimarySep3D));

	   std::cout << "r_trk=" <<  rho_trk
		<< " r_cas="<< rho_cas
		<< " r_cas2="<< XiVtxPos.perp()
//                << " chi2="<< chi2
//                << " ndof="<< ndof
//                << " prob="<< probvxi
                << " trk_chisq="<< trkchi2
...
void 
CascadeNtuplizer::endJob() {
}

//define this as a plug-in
DEFINE_FWK_MODULE(CascadeNtuplizer);
