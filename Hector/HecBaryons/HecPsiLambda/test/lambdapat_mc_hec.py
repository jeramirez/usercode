...
from HecBaryons.HecPsiLambda.Cert_160404_180252_7TeV_PromptReco_Collisions11_CMSSWConfig_MuonPhys_Hec_cfi import *
...
options.maxEvents = 200              # -1 means all event
...
process = cms.Process("MyHecPsiLambdaPAT")
...
#--eric process.muonMatch.matched = cms.InputTag("genParticlesPlusSim")
...
if (runMC):
   ...
   #--eric process.patTrackCandsMCMatch.matched = cms.InputTag("genParticlesPlusSim")
...
process.generalV0Candidates.innerHitPosCut = cms.double(-1)   #<--for AOD data
process.MyLambda0 = process.generalV0Candidates.clone()
process.MyLambda0.vtxSignificance2DCut  = cms.double(5.0)    #--default is 15
process.MyLambda0.impactParameterSigCut = cms.double(0.5)    #--default is 2


#if (runMC):

process.HecJL   = cms.EDAnalyzer('HecPsiLambdaPAT'
...
#   , MuonsLabel = cms.InputTag('muons')
   ...
   , MyPrintMC  = cms.untracked.bool(True)
   , VeeAlgo    = cms.untracked.string("MyLambda0")  #--Collection Label decay (Lambda) (Eduardo}
...
process.gp = cms.Path( process.genParticlesRemake * process.genParticlesPlusSim )

process.phec = cms.Path(process.trigMatch*process.MyLambda0*process.HecJL)
process.trigMatchPath = cms.Path( process.trigMatch )

#process.schedule = cms.Schedule(process.pat, process.trigMatchPaths)
#--eric process.schedule = cms.Schedule(process.gp, process.pat, process.trigMatchPath, process.phec )
process.schedule = cms.Schedule(process.gp, process.pat, process.trigMatchPath, process.phec )
